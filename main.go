...
func (r Running) Calories() float64 {
	...
	duration := r.Duration.Hours() // длительность тренировки в часах
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speed, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * durationInHours * MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	timeDuration := float64(s.Duration)
	if timeDuration == 0 {
		return 0
	}
	return float64(s.LengthPool) * float64(s.CountPool) / MInKm / timeDuration
}
